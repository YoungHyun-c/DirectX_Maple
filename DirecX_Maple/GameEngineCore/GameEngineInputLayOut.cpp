...
UINT GameEngineInputLayOutInfo::FormatSize(DXGI_FORMAT _Format)
{
	...
	MsgBoxAssert("처리할 수 없는 포맷을 사용헀습니다.");
	return -1;
}

void GameEngineInputLayOutInfo::AddInputLayOutDesc(
	LPCSTR _SemanticName, // 내가 포지션이다 내가 컬러다. = "POSITION"
	DXGI_FORMAT _Format, // 내가 n바이트 짜리 정보다 자료형 = DXGI_FORMAT::R32G32
	UINT _SemanticIndex/* = 0*/, // POSIONT0
	UINT _AlignedByteOffset /* = -1*/, // 0번째 바이트부터
	D3D11_INPUT_CLASSIFICATION _InputSlotClass /*= D3D11_INPUT_PER_VERTEX_DATA*/, // 버텍스 데이터가 인스턴싱 데이터다 라는 걸 표현
	UINT _InputSlot /* = 0*/, // n개와 버텍스 버퍼를 세팅할때 n 번째 버텍스 버퍼의 인풋 레이아웃이다 라는걸 표현.
	UINT _InstanceDataStepRate /* = 0*/ // 나중에 인스턴싱이라는 구조를 배울 때 쓸때가 있다.
)
{
	...
	std::shared_ptr<class GameEngineVertexShader> _Shader)
{
	...
	// pShaderBytecodeWithInputSignature 쉐이더 코드 달라고 하는 것
