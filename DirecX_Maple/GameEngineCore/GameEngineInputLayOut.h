...
public:
	...
	// 인스턴싱은 다수의 동일한 설정을 가진 렌더링 세트를 한번에 그려서 속력을 취하는 것을 말한다.

	void AddInputLayOutDesc(
		LPCSTR _SemanticName, // 내가 포지션이다 내가 컬러다. = "POSITION"
		DXGI_FORMAT _Format, // 내가 n 바이트 짜리 정보다 자료형 = DXGI_FORMAT::R32G32 32
		UINT _SemanticIndex = 0, // POSITION0
		UINT _AligendByteOffset = -1, // 0번째 바이트부터.
		D3D11_INPUT_CLASSIFICATION _InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA, // 버텍스 데이터가 인스턴싱 데이터다 라는걸 표현
		UINT _InputSlot = 0, // n 개의 버텍스 버퍼를 세팅할때 n 번째 버텍스 버퍼의 인풋 레이아웃이다 라는걸 표현하는 것
