...
void GameEngineDevice::Initiallize(const GameEngineWindow& _Window)
{
	...
	// 다이렉트X 버전 몇까지 지원하는지 알아내가 위한 변수.
	D3D_FEATURE_LEVEL Level = D3D_FEATURE_LEVEL_11_0;

	// 그래픽카드의 기본정보를 알려주고 제어할 수 있는 인터페이스.
	IDXGIAdapter* Adapter = GetHighPerformanceAdapter();

	if (nullptr == Adapter)
	{
		Adapter->Release();
		MsgBoxAssert("그래픽 카드가 없는 컴퓨터입니다.");
		return;
	}

	// D11 디바이스는 그래픽카드 랜더링 메모리를 제어할 수 있는 인터페이스입니다.
	// 최중요 인터페이스 => 메모리

	// 이 장치의 IDXGIAdapter* Adapter
	// 메모리에 접근하는 권한과 ID3D11Device* Device
	// 그림을 그릴 수 있는 권한을 만들어줘. ID3D11DeviceContext* Context

	// 0 넣으면 기본으로 해줘.

	HRESULT Result = D3D11CreateDevice(
		Adapter, // 장치 <= 장치에서 그래픽카드의 정보를 불러온다
		D3D_DRIVER_TYPE::D3D_DRIVER_TYPE_UNKNOWN, // 불러온 장치 쓸꺼니 모름.
		nullptr, // 모름
		iFlag, // 초기화 옵션
		nullptr, // 사용할 수 있는 펙처레벨
		0, // 그중에서 내가 원하는 펙처레벨 할 수 있는
	...
	if (0 == Result)
	{
		MsgBoxAssert("멀티쓰레드 모드로 그래픽카드를 이용할 수 없습니다.");
		return;
	}

	// 이러고 나면
	// 렌더링의 구조는 바뀌지 않는다.
	// 다이렉트 3D나 WinApi나 다르지 않다.
...
IDXGIAdapter* GameEngineDevice::GetHighPerformanceAdapter()
{
	...
	if (nullptr == Factory)
	{
		MsgBoxAssert("그래픽 카드에서 팩토리 인터페이스를 생성하지 못 했습니다.");
		return nullptr;
	}
...
{
	// 스왑체인에 연결된 텍스처에 그려져있는 색상을 화면에 출력하라는 것.
